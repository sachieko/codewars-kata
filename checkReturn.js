...
(you don't want to be early or late!) and will, of course,
 ...
 const checkReturn = function(walk) {
  const stepCounter = {};
  for (const step of walk) {
    stepCounter[step] ? stepCounter[step] += 1 : stepCounter[step] = 1;
  }
  return (stepCounter['n'] === stepCounter['s'] && stepCounter['w'] === stepCounter['e']);
};
const isValidWalk = function(walk) {
  return (walk.length === 10 && checkReturn(walk)) ? true : false;
};